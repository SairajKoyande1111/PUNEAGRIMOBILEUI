...
import { extractAadhar, extractPassbook } from "../lib/datalab";
import { setAadharOnUser, setPassbookOnUser } from "../lib/mongo";
...
router.post("/ocr/passbook", async (req, res) => {
  const body = req.body as
    | { phone?: string; imageBase64?: string; mimeType?: string }
    | undefined;
  const phone = String(body?.phone ?? "").replace(/\D/g, "").trim();
  ...
  try {
    ...
    req.log.info({ phone, size: buffer.length }, "Running passbook OCR");

    const ocr = await extractPassbook(buffer, mimeType);
    const doc = await setPassbookOnUser(phone, ocr);
    ...
    req.log.error({ err: e }, "Passbook OCR failed");
    const message = e instanceof Error ? e.message : "OCR failed";
    res.status(500).json({ error: message });
  }
});
